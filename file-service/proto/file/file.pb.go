...
func (m *FileRequest) Marshal() ([]byte, error) {
	...
	if m.Filename != "" {
		buf = append(buf, 0x0a) // field 1, string type
		buf = append(buf, encodeVarint(uint64(len(m.Filename)))...)
		buf = append(buf, m.Filename...)
	}
	// Field 2: Hash (string)
	if m.Hash != "" {
		buf = append(buf, 0x12) // field 2, string type
		buf = append(buf, encodeVarint(uint64(len(m.Hash)))...)
		buf = append(buf, m.Hash...)
	}
	// Field 3: Size (int64)
	if m.Size != 0 {
		buf = append(buf, 0x18) // field 3, varint type
		buf = append(buf, encodeVarint(uint64(m.Size))...)
	}
	// Field 4: UploadTime (string)
	if m.UploadTime != "" {
		buf = append(buf, 0x22) // field 4, string type
...
func (m *FileResponse) Marshal() ([]byte, error) {
	...
	if m.Success {
		buf = append(buf, 0x08) // field 1, varint type
		buf = append(buf, 0x01)
	}
